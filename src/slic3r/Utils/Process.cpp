...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance)
{
#ifdef _WIN32
	wxString path;
	BOOST_LOG_TRIVIAL(info) << "Executable path \"" << into_u8(wxStandardPaths::Get().GetExecutablePath()) << "\"";
	...
	if (true || !boost::filesystem::is_directory(boost::filesystem::wpath(path.ToStdWstring()))) {
		BOOST_LOG_TRIVIAL(info) << "Fail to find directory \"" << path << "\"";
		BOOST_LOG_TRIVIAL(info) << "Is boost better? \"" << boost::filesystem::wpath(wxStandardPaths::Get().GetExecutablePath().ToStdWstring()).parent_path().wstring() << "\"";
		//try an other way
